...
import rawConfig from "../config/events.json" with {type:"json"};import {buildCatalogSnapshot,preserveLastKnownResults,type CatalogSnapshot} from "./catalog.ts";import {validateConfig} from "./config.ts";import type {Env,ExecutionContextLike} from "./env.ts";export {RaceCatalogCache} from "./race-cache.ts";import type {RefreshStatus} from "./race-cache.ts";
...
export async function refreshCatalog(env:Env,trigger:"scheduled"|"manual"="scheduled"):Promise<void>{const stub=env.RACE_CATALOG.getByName("catalog");const startedAt=new Date().toISOString();await writeRefreshStatus(stub,{state:"running",trigger,startedAt});try{const previousResponse=await stub.fetch("https://catalog/snapshot");const previous=previousResponse.ok?await previousResponse.json() as CatalogSnapshot:undefined;const snapshot=await buildCatalogSnapshot(validateConfig(rawConfig).events,env,fetch,new Date(),previous?.results);if(previous&&snapshot.failedIds.length){const failed=new Set(snapshot.failedIds);snapshot.events.push(...previous.events.filter(event=>failed.has(event.id)))}snapshot.results=preserveLastKnownResults(snapshot.results,previous?.results);const response=await stub.fetch("https://catalog/snapshot",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(snapshot)});if(!response.ok)throw new Error(`Catalog snapshot write returned HTTP ${response.status}`);await writeRefreshStatus(stub,{state:"success",trigger,startedAt,completedAt:new Date().toISOString(),failures:snapshot.failures,failureDetails:snapshot.failureDetails})}catch(error){const message=error instanceof Error?error.message:"Unknown refresh error";await writeRefreshStatus(stub,{state:"failure",trigger,startedAt,completedAt:new Date().toISOString(),error:message});throw error}}
async function refreshStatus(env:Env):Promise<RefreshStatus|undefined>{const response=await env.RACE_CATALOG.getByName("catalog").fetch("https://catalog/refresh-status");return response.ok?await response.json() as RefreshStatus:undefined}
async function writeRefreshStatus(stub:Env["RACE_CATALOG"] extends {getByName(name:string):infer T}?T:never,status:RefreshStatus):Promise<void>{const response=await stub.fetch("https://catalog/refresh-status",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(status)});if(!response.ok)throw new Error(`Refresh status write returned HTTP ${response.status}`)}
...
async function handleAdmin(request:Request,env:Env,pathname:string):Promise<Response>{
  ...
  const token=newCsrf();
  if(request.method==="GET"&&pathname==="/admin")return html(adminPage(token,"",await refreshStatus(env)),200,csrfCookie(token));
  ...
  try{
    if(pathname==="/admin/refresh"){await refreshCatalog(env,"manual");return html(adminPage(token,"Catalog refresh completed.",await refreshStatus(env)),200,csrfCookie(token));}
...
export default {fetch:handleRequest,scheduled(_controller:unknown,env:Env,context:ExecutionContextLike){context.waitUntil(refreshCatalog(env,"scheduled"));}};
