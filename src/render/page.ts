...
function card(event: NormalizedEvent): string {
  const localDate = event.startDateTime.slice(0, 10); const date = new Date(`${localDate}T12:00:00Z`); const month = new Intl.DateTimeFormat("en-US",{month:"short",timeZone:"UTC"}).format(date); const day = new Intl.DateTimeFormat("en-US",{day:"2-digit",timeZone:"UTC"}).format(date);
  ...
  const image = event.imageUrl ? `<img class="event-image" src="${escapeHtml(event.imageUrl)}" alt="" loading="lazy" width="240" height="180">` : "";
  return `<article class="card"><div class="date" aria-label="${escapeHtml(month)} ${escapeHtml(day)}"><span>${escapeHtml(month)}</span><strong>${escapeHtml(day)}</strong></div>${image}<div class="content"><h2>${escapeHtml(event.name)}</h2><p class="meta">${where}</p>${distances}<a class="action" href="${escapeHtml(event.registrationUrl)}" aria-label="View event: ${escapeHtml(event.name)}">View event</a></div></article>`;
}
