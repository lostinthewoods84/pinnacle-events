...
import type { RefreshStatus } from "./race-cache.ts";
...
export type ResultsDiagnosticReport = {raceId:number;raceName:string;metadataMs:number;settingsMs?:number;externalResultLinks?:Array<{name:string;officialUrl:string}>;settingsError?:string;events:Array<{eventId:string;eventName:string;durationMs:number;status:"ok"|"error";resultSets:Array<{id:number;name:string;officialUrl:string}>;payload?:ResultPayloadDiagnostic;error?:string}>};
...
export function adminPage(csrf: string, message = "", status?: RefreshStatus): string {
  let refresh="<p class=\"message\">No refresh status has been recorded yet.</p>";
  if(status){const heading=status.state==="running"?"Running":status.state==="success"?"Last refresh succeeded":"Last refresh failed";const completed=status.completedAt?` · Completed: ${escapeHtml(new Date(status.completedAt).toLocaleString("en-US"))}`:"";const count=status.failures!==undefined?` · Race failures: ${status.failures}`:"";const error=status.error?`<p class="message">${escapeHtml(status.error)}</p>`:"";const details=(status.failureDetails??[]).map(failure=>`<li>${escapeHtml(failure.catalogId)}${failure.raceId?` (${failure.raceId})`:""}: ${escapeHtml(failure.error)}</li>`).join("");refresh=`<section class="preview"><p class="eyebrow">Catalog refresh</p><h2>${heading}</h2><p>Started: ${escapeHtml(new Date(status.startedAt).toLocaleString("en-US"))}${completed}</p><p>Trigger: ${escapeHtml(status.trigger)}${count}</p>${error}${details?`<ul>${details}</ul>`:""}</section>`;}
  return shell("Pinnacle administration", `<h1>Pinnacle administration</h1>${message ? `<p class="message" role="status">${escapeHtml(message)}</p>` : ""}${refresh}<form method="post" action="/admin/refresh"><input type="hidden" name="csrf" value="${escapeHtml(csrf)}"><button type="submit">Refresh catalog now</button></form><h2>Add a RunSignup event</h2><p>Paste the RunSignup dashboard URL, public race URL, or race ID. Nothing is published until the resulting GitHub pull request is merged.</p><form method="post" action="/admin/preview"><input type="hidden" name="csrf" value="${escapeHtml(csrf)}"><label for="race">RunSignup URL or race ID</label><input id="race" name="race" required autocomplete="off" placeholder="https://runsignup.com/Race/Dashboard/Overview/205693"><button type="submit">Preview event</button></form><h2>Test published results</h2><p>Use a completed race with published results. This makes live, sequential RunSignup requests and does not modify the catalog.</p><form method="post" action="/admin/results-test"><input type="hidden" name="csrf" value="${escapeHtml(csrf)}"><label for="results-race">Completed RunSignup race ID</label><input id="results-race" name="race" required autocomplete="off" inputmode="numeric" placeholder="205693"><button type="submit">Test results API</button></form>`);
}

export function resultsDiagnosticPage(report:ResultsDiagnosticReport):string{const rows=report.events.map(event=>`<section class="preview"><p class="eyebrow">Event ${escapeHtml(event.eventId)} · ${event.durationMs} ms</p><h2>${escapeHtml(event.eventName)}</h2><p><strong>${event.status==="ok"?"Request succeeded":"Request failed"}</strong></p>${event.error?`<p class="message">${escapeHtml(event.error)}</p>`:""}${event.resultSets.length?`<ul>${event.resultSets.map(set=>`<li><a href="${escapeHtml(set.officialUrl)}">${escapeHtml(set.name)}</a> <span class="eyebrow">Result set ${set.id}</span></li>`).join("")}</ul>`:`<p>No public result sets returned.</p>`}${event.payload?renderPayloadDiagnostic(event.payload):""}</section>`).join("");const external=report.externalResultLinks?.length?`<section class="preview"><p class="eyebrow">Result settings · ${report.settingsMs??0} ms</p><h2>External results</h2><ul>${report.externalResultLinks.map(link=>`<li><a href="${escapeHtml(link.officialUrl)}">${escapeHtml(link.name)}</a></li>`).join("")}</ul></section>`:report.settingsError?`<p class="message">External result-link request failed: ${escapeHtml(report.settingsError)}</p>`:`<p>External result links returned: 0${report.settingsMs!==undefined?` · ${report.settingsMs} ms`:""}</p>`;return shell("Results API diagnostic",`<a href="/admin">← Back to administration</a><h1>Results API diagnostic</h1><p><strong>${escapeHtml(report.raceName)}</strong> · RunSignup race ${report.raceId}</p><p>Race metadata request: ${report.metadataMs} ms</p>${rows||"<p>No events were returned for this race.</p>"}${external}`)}
