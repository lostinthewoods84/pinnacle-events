...
export function adminPage(csrf: string, message = "", status?: RefreshStatus): string {
  const refresh=status?`<section class="preview"><p class="eyebrow">Catalog refresh</p><h2>${status.state==="running"?"Running":status.state==="success"?"Last refresh succeeded":"Last refresh failed"}</h2><p>Started: ${escapeHtml(new Date(status.startedAt).toLocaleString("en-US"))}${status.completedAt?` · Completed: ${escapeHtml(new Date(status.completedAt).toLocaleString("en-US"))}`:""}</p><p>Trigger: ${escapeHtml(status.trigger)}${status.failures!==undefined?` · Race failures: ${status.failures}`:""}</p>${status.error?`<p class="message">${escapeHtml(status.error)}</p>`:""}${status.failureDetails?.length?`<ul>${status.failureDetails.map(failure=>`<li>${escapeHtml(failure.catalogId)}${failure.raceId?` (${failure.raceId})`:""}: ${escapeHtml(failure.error)}</li>`).join("")}</ul>`:""}</section>`:"<p class="message">No refresh status has been recorded yet.</p>";
